...
/*--------------------------------------------------------------------------------------*/
-- --------------------------------------------------------------------
/*1- Write an SQL query to calculate the total revenue generated by the company*/
-- ------------------------------------------------------------------

SELECT round(SUM(p.Price * od.Quantity),2) AS TotalRevenue
FROM products AS p
JOIN order_details AS od ON p.ProductID = od.ProductID;

-- ----------------------------------------------------------------
/*2- Write an SQL query to find the top 5 customers who have made the highest
total_amount of purchases. Include their CustomerID, CustomerName, and the total
amount spent*/
SELECT c.CustomerID,c.CustomerName,p.price
,round(SUM(p.price),2) as total_purchase
FROM customers AS c
JOIN orders as o
...
LIMIT 5;
-- ---------------------------------------------------------
/*3- Write an SQL query to calculate the average order total_amount for each month of the
year. The result should include the month and the average total_amount*/
-- ---------------------------------------------------------
select extract(month from o.orderdate) as month, avg(od.quantity) as average_orders
from orders as o
join order_details as od
...
order by month asc;
-- --------------------------------------------------------
/*4- Write an SQL query to identify the top-selling product category based on the total
number of orders. Include the category and the count of orders.*/
-- -------------------------------------------------------
select p.categoryid, ca.categoryname, sum(od.quantity) as count_of_orders
from products as p
join order_details as od
on p.productid = od.productid
...
-- ----------------------------------------------------------
/*5- Write an SQL query to calculate the total revenue generated by each product category.
The result should include the category and the total revenue*/
-- ----------------------------------------------------------

select p.categoryid, ca.categoryname , sum(p.price) as total_revenue
from products as p 
join categories as ca
on p.categoryid=ca.categoryid
group by 
...
-- -------------------------------------------------------------
/*6-Write an SQL query to find the month with the highest revenue and the total revenue.*/
-- -------------------------------------------------------------

select extract(month from o.orderdate) as month, round(sum(p.price),2) as revenue
from orders as o 
join order_details as od
on od.orderid=o.orderid
join products as p
...
-- ---------------------------------------------------------------
/*7- Write an SQL query to calculate the customer retention rate. Consider customers who
have made at least one purchase in the previous year as retained customers.*/
-- ---------------------------------------------------------------

with retained_customers as (
select c.customername, p.price, extract(year from o.orderdate) as year
from customers as c
...
Select round((count(r.price)*100/count(r.price is not null)),2) as retention_rate
from retained_customers as r;

-- ---------------------------------------------------------------------------
/*8- Write an SQL query to calculate the average order value for each customer, considering
only customers who have made more than three purchases. Include the CustomerID,
CustomerName, and the average order value.*/
-- ---------------------------------------------------------------------------
select c.customerid,c.customername,round(avg(p.price*od.quantity),2) as avg_ord_val
from customers as c
left join orders as o
on c.customerid = o.customerid
left join order_details as od
...
-- ----------------------------------------------------------------------------
/*9-Write an SQL query to find the top 3 customers who have made the highest average
purchase amount per order. Include their CustomerID, CustomerName, and the average
purchase amount.*/
select c.customerid, c.customername,round(avg(p.price*od.quantity),2) as avg_purchase_amount
from customers as c
join orders as o 
...
/*10-Write an SQL query to calculate the monthly revenue growth rate. Compare the revenue
...
SELECT
    year_and_month,
    Round((total_current_month_growth-total_last_month_growth)*100/total_last_month_growth,4) as growth_rate
FROM (
    SELECT
    DATE_FORMAT(orderdate, '%Y-%m') AS year_and_month,
        LAG(sum(p.price * od.quantity)) 
        OVER (ORDER BY DATE_FORMAT(orderdate, '%Y-%m'))
        AS total_last_month_growth,
        sum(p.price * od.quantity) AS total_current_month_growth
    FROM orders AS o
...
-- -------------------------------------------------------------------------------
/*11-Write an SQL query to identify the top-selling product in each product category based on
the total revenue generated. Include the category, ProductName, and total revenue*/

select cat.categoryname,p.productname,sum(p.price*od.quantity) as total_revenue
...
-- -------------------------------------------------------------------------------
/*12-Write an SQL query to calculate the customer lifetime value (CLV) for each customer.
...
select c.customerid, c.customername, round(avg(p.price*od.quantity),2) as CLV
from customers as c
join orders as o 
on o.customerid=c.customerid
join order_details as od
...
-- ---------------------------------------------------------------------
/*13 - Write an SQL query to identify the top 5 customers who have shown the highest growth
in total_amount spent compared to the previous year. Include their CustomerID,
CustomerName, and the growth percentage.*/
-- ---------------------------------------------------------------------

select customerID,customerName,round((total_1997-total_1996)*100/total_1996,2) as growth_rate
from(
select c.customerid as customerID,c.customername as customerName,
round(SUM(CASE WHEN YEAR(o.orderdate) = 1996 THEN p.price * od.quantity ELSE 0 END),2) AS total_1996,
round(SUM(CASE WHEN YEAR(o.orderdate) = 1997 THEN p.price * od.quantity ELSE 0 END),2) AS total_1997
from customers as c
join orders as o 
on o.customerid=c.customerid
join order_details as od
...
-- --------------------------------------------------------------------
/*14-Write an SQL query to calculate the average time between a customer's first and second
...
select
 ...
 datediff(MIN(CASE WHEN ro.order_rank = 2 THEN ro.orderdate END),
 MIN(CASE WHEN ro.order_rank = 1 THEN ro.orderdate END)) as avg_days_difference
 /*By using MIN(),
 we ensure that we are selecting the earliest dates for the first and second purchases,
 which is what we typically want when determining a customer's initial and subsequent purchases.*/
FROM
  customers AS c
  left JOIN rankorders AS ro ON c.customerID = ro.customerID
  group by c.customerid) as sub
  where avg_days_difference is not null;
  
-- ---------------------------------------------------------------
/*15 - Write an SQL query to find the customers who have not made any purchases in the last
six months. Include their CustomerID, CustomerName, and the date of their last
purchase.*/
-- ---------------------------------------------------------------
SELECT
    c.customerid,
    c.customername,
    o.orderdate as Last_date_of_purchase
FROM
    customers AS c
...
/*16- Write an SQL query to calculate the average number of orders per day of the week
(Monday, Tuesday, etc.). The result should include the day of the week and the average
number of orders.*/
-- ----------------------------------------------

select day(o.orderdate) as day, avg(od.quantity) as avg_no_of_orders
from orders as o
join order_details as od
on o.orderid=od.orderid
group by day 
...
-- ---------------------------------------------------
/*17- Write an SQL query to identify the top 5 months with the highest number of new
customers (customers who joined in that month). Include the month and the count of
new customers. */
/* I need a column for customers who have made their first purchase
and a column for the month which they did it and the number of customers*/
...
GROUP BY
    month
ORDER BY
    new_customer_count DESC
LIMIT 5;


-- -------------------------------------------------------
/*18-Write an SQL query to calculate the total revenue generated by each customer in their
first year. Include the CustomerID, CustomerName, and the total revenue.*/
 -- ------------------------------------------------------
...
-- --------------------------------------------------------------
/*19-Write an SQL query to find the customers who have made consecutive purchases for
three or more months. Include their CustomerID, CustomerName, and the count of
consecutive months.*/
...
WITH RankedOrders AS (
    SELECT
        ...
        DENSE_RANK() OVER (PARTITION BY c.CustomerID ORDER BY YEAR(o.OrderDate), MONTH(o.OrderDate)) AS PurchaseRank
    FROM
        Customers AS c
    JOIN 
        Orders AS o ON c.CustomerID = o.CustomerID
...
-- -------------------------------------------------
/*20- Write an SQL query to calculate the average order value for each product category.
Include the category and the average order value.. */
-- ---------------------------------------------------

SELECT
    c.CategoryName AS Category,
    round(AVG(p.price * od.quantity),2) AS AvgOrderValue
FROM
    order_details AS od
JOIN
